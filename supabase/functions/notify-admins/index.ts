...
function adminTemplate(heading: string, intro: string, rows: Array<[string, string]>): string {
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
    <table role="presentation" width="520" cellpadding="0" cellspacing="0" style="background:#fff;border:1px solid #e2ebe4;border-radius:14px;overflow:hidden">
      <tr><td style="background:#1d7733;padding:16px 24px;color:#fff;font-size:13px;font-weight:700;letter-spacing:1px">IFFS 2027 BIANNUAL SURVEY</td></tr>
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
    <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="background:#fff;border:1px solid #e2ebe4;border-radius:16px;overflow:hidden">
      <tr><td align="center" style="background:#1d7733;padding:24px">
        ...
        <div style="color:#fff;font-size:13px;font-weight:700;letter-spacing:2px;margin-top:10px">IFFS 2027 BIANNUAL SURVEY</div>
      </td></tr>
      <tr><td style="padding:28px 32px">
        ...
        <p style="margin:0 0 14px;font-size:15px;color:#3d4a52;line-height:1.6">
          Your response to the IFFS 2027 Biannual Survey has been received. On behalf of the
      ...
      <tr><td style="padding:16px 32px;border-top:1px solid #eef2ef;color:#b0bec5;font-size:11px">
        International Federation of Fertility Societies · 2027 Biannual Survey on ART
      </td></tr>
    </table>
  </td></tr></table>
  </body></html>`
...
serve(async (req: Request) => {
  ...
  if (body.type === 'signup') {
    ...
    const r = await sendEmail(
      ...
      adminTemplate(
        'New user registered',
        'A new participant has created an account on the IFFS 2027 Biannual Survey.',
        [
          ['Name', name],
          ['Email', body.email ?? ''],
        ]
  ...
  if (body.type === 'completion') {
    ...
    if (p?.email) {
      const r = await sendEmail(
        resendKey,
        [p.email as string],
        'Thank you for completing the IFFS 2027 Biannual Survey',
  ...
  if (body.type === 'password_reset') {
    ...
    const r = await sendEmail(
      ...
      adminTemplate(
        'Password reset requested',
        'A user has requested a password reset link for their IFFS 2027 Biannual Survey account.',
        [
          ['Name', name],
          ['Email', body.email ?? ''],
